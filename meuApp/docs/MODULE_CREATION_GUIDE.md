...
- [Passo 6: Commands](#passo-6-commands)
...
var (
    ...
    ErrCategoryAlreadyExists = errors.NewConflictError("CATEGORY_ALREADY_EXISTS", "category already exists")
    ErrInvalidCategoryData   = errors.NewValidationError("INVALID_CATEGORY_DATA", "invalid category data")
    ErrCategoryNameRequired  = errors.NewValidationError("CATEGORY_NAME_REQUIRED", "category name is required")
    ErrCategoryInactive      = errors.NewBusinessError("CATEGORY_INACTIVE", "category is inactive")
)
```
...
func (h *UpdateCategoryHandler) Execute(ctx context.Context, id string, req dto.UpdateCategoryRequest) (*dto.CategoryResponse, error) {
    ...
    h.logger.Info("Category updated", "category_id", id)
...
func (h *CategoryHandler) CreateCategory(c *gin.Context) {
    ...
    if err := c.ShouldBindJSON(&req); err != nil {
        appErr := errors.NewValidationError("INVALID_REQUEST", err.Error())
...
func NewCategoryGRPCService(appService *services.CategoryApplicationService) *CategoryGRPCService {
    return &CategoryGRPCService{appService: appService}
}

func (s *CategoryGRPCService) CreateCategory(ctx context.Context, req *pb.CreateCategoryRequest) (*pb.CategoryResponse, error) {
...
func (s *CategoryGRPCService) GetCategory(ctx context.Context, req *pb.GetCategoryRequest) (*pb.CategoryResponse, error) {
...
func (m *CategoryModule) Register(registry *container.ModuleRegistry) error {
    ...
    registry.RegisterHTTPHandler("category", httpHandler)
    
    // 6. gRPC Service
    grpcService := grpc.NewCategoryGRPCService(appService)
    registry.RegisterGRPCService("category", grpcService)
...
func registerModules(registry *container.ModuleRegistry, db *gorm.DB, eventBus contracts.EventPublisher, logger contracts.Logger) error {
    ...
    // Category Module
    categoryModule := modules.NewCategoryModule(db, eventBus, logger)
    if err := categoryModule.Register(registry); err != nil {
        return fmt.Errorf("failed to register category module: %w", err)
    }

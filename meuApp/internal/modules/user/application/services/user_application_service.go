...
func (s *UserApplicationService) ListUsers(ctx context.Context, page, pageSize int) (*ports.PaginatedUserResult, error) {
	query := queries.ListUsersQuery{Page: page, PageSize: pageSize}
	result, err := s.listUsersHandler.Handle(ctx, query)
	if err != nil {
		return nil, err
	}

	// Converter ListUsersResult para PaginatedUserResult

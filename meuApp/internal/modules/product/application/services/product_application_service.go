...
// ListProductsPaginated lista produtos com filtros e paginação
func (s *ProductApplicationService) ListProductsPaginated(ctx context.Context, filters ports.ProductFilters) (*ports.PaginatedResult, error) {
	query := queries.ListProductsQuery{
		CategoryID: filters.CategoryID,
		MinPrice:   filters.MinPrice,
		MaxPrice:   filters.MaxPrice,
	...
	return s.listHandler.HandlePaginated(ctx, query)
}
